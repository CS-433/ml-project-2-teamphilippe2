...
def train(model, criterion, dataset_train, dataset_test, device, optimizer, num_epochs, print_iteration=True,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_x, batch_y in dataset_train:
            ...
            # If we are using an autoencoder, should give
            # input image as target
            if autoencoder:
...
def run_experiment(model_str, loss_fct_str, optimizer_str, image_dir, gt_dir, num_epochs=10,
                   learning_rate=1e-3, momentum=0.0, batch_size=100, save_weights=True,
                   ratio_test=0.2, seed=1, autoencoder=False):
    """
    ...
    train(model, criterion, dataset_train, ds.get_test_set(), device, optimizer, num_epochs, autoencoder=autoencoder)
